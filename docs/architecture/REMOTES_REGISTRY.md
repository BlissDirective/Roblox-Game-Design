...
| `StructureHealth` | Server → Client (Event) | `(update: StructureHealthService.HealthUpdate)` — fired only to the structure's owner | n/a (server-driven) | R1 | Carries `buildableId`, `cellX/cellZ`, world `position`, `hp/maxHp`, and a `state` enum (`"damaged"` / `"destroyed"` / `"disabled"` / `"repaired"`). Fires when an alien damages a structure, an extractor is disabled/destroyed by a lost wave, or the player repairs. Client renders damage-state visuals + repair affordance; HP authority lives on the Part attributes + `StructureHealthService`. |
| `RepairStructure` | Client → Server (Event) | `(cellX: integer, cellZ: integer)` — cells range-checked against `PlotSlotSize` | `RATE_LIMITS.RepairStructure` (10/sec — hold-to-repair) | R1 | Server validates plot ownership, that a damaged tracked structure sits at that cell, player is within `COMBAT.RepairReachStuds`, and can afford `max(RepairCostMin, missingHp × RepairCostPerHp)`. Restores to full HP and re-enables a disabled extractor. Fire-and-forget; the resulting `StructureHealth` "repaired" push confirms. |
...
| `RepairStructure` | `RepairStructure` | 10 / s | 20 |
